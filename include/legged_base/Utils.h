...
inline std::string getEnv(const std::string &env_name) {
  ...
  if (!env_value) {
    throw std::runtime_error("Environment variable '" + env_name +
                             "' is not set");
  }
...
inline double smoothLerp(double t, const double &x0, const double &x1,
                         size_t order = 5) {
...
smoothLerp(double t, const Eigen::MatrixBase<DerivedA> &x0,
           const Eigen::MatrixBase<DerivedB> &x1, size_t order = 5) {
  return (x0 + (x1 - x0) * smoothStep(t, order))
      .eval(); // return a plain (materialized) vector
}

// [xyzw]
inline Eigen::VectorXd baseSlerp(double t, const Eigen::VectorXd &qBaseDes,
                                 const Eigen::VectorXd &qBaseInit,
                                 size_t order = 5) {
  // 1. Position interpolation
  Eigen::Vector3d base_pos =
      smoothLerp(t, qBaseInit.head<3>(), qBaseDes.head<3>(), order).eval();

  // 2. Orientation interpolation
  Eigen::Quaterniond quat_init(qBaseInit[6], qBaseInit[3], qBaseInit[4],
                               qBaseInit[5]); // w,x,y,z
  Eigen::Quaterniond quat_des(qBaseDes[6], qBaseDes[3], qBaseDes[4],
                              qBaseDes[5]); // w,x,y,z
  quat_init.normalize();
  quat_des.normalize();
  ...
  Eigen::Quaterniond quat_interp =
      quat_init.slerp(smoothStep(t, order), quat_des);
...
inline Eigen::VectorXd concatVectors(const std::vector<VecT> &vecs) {
  static_assert(Eigen::internal::traits<VecT>::ColsAtCompileTime == 1,
...
//
template <typename VecT>
inline std::vector<VecT> splitVectors(const Eigen::VectorXd &big,
...
// 输出：
//   vec_out    - 输出向量 (dim * n)
inline void reorder(const std::vector<std::string> &names_in,
  ...
  if (vec_in.size() % n_in != 0)
    throw std::invalid_argument(
        "vec_in size must be divisible by names_in.size()");
  ...
  for (int i = 0; i < n_out; ++i) {
    ...
    if (it == name_to_idx.end())
      throw std::invalid_argument("Name " + names_out[i] +
                                  " not found in names_in");
...
inline void reorder_rows(const std::vector<std::string> &names_in,
  ...
  if (n_in != n_out)
    throw std::invalid_argument(
        "reorder_rows: names_in and names_out must have same length");
  if (n_in == 0)
    throw std::invalid_argument("reorder_rows: names_in is empty");
  if (mat_in.rows() % n_in != 0)
    throw std::invalid_argument(
        "reorder_rows: mat_in.rows() must be divisible by names_in.size()");
  ...
  if (mat_out.rows() != mat_in.rows() || mat_out.cols() != mat_in.cols())
    throw std::invalid_argument(
        "reorder_rows: mat_out must have same shape as mat_in");
  ...
  for (int i = 0; i < n_out; ++i) {
    auto it = name_to_idx.find(names_out[i]);
    if (it == name_to_idx.end())
      throw std::invalid_argument("reorder_rows: Name " + names_out[i] +
                                  " not found in names_in");
...
inline void reorder_cols(const std::vector<std::string> &names_in,
                         const Eigen::Ref<const Eigen::MatrixXd> &mat_in,
                         const std::vector<std::string> &names_out,
                         Eigen::Ref<Eigen::MatrixXd> mat_out) {
  const int n_in = static_cast<int>(names_in.size());
  ...
  if (n_in != n_out)
    throw std::invalid_argument(
        "reorder_cols: names_in and names_out must have same length");
  if (n_in == 0)
    throw std::invalid_argument("reorder_cols: names_in is empty");
  if (mat_in.cols() % n_in != 0)
    throw std::invalid_argument(
        "reorder_cols: mat_in.cols() must be divisible by names_in.size()");
  ...
  if (mat_out.rows() != mat_in.rows() || mat_out.cols() != mat_in.cols())
    throw std::invalid_argument(
        "reorder_cols: mat_out must have same shape as mat_in");
  ...
  for (int i = 0; i < n_out; ++i) {
    auto it = name_to_idx.find(names_out[i]);
    if (it == name_to_idx.end())
      throw std::invalid_argument("reorder_cols: Name " + names_out[i] +
                                  " not found in names_in");
